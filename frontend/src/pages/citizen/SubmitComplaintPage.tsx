...
export function SubmitComplaintPage() {
  ...
  const [audioDataUri, setAudioDataUri] = useState<string>()  // base64 data URI for storage
  const mediaRef  = useRef<MediaRecorder | null>(null)
  const chunksRef = useRef<Blob[]>([])
    ...
    try {
      ...
      mr.onstop = async () => {
        ...
        try {
          ...
          // ── NEW FLOW: one request does upload + transcription ──────────────
          // Backend receives audio bytes → Groq Whisper transcribes → returns
          // both transcript AND base64 data URI. No R2, no second hop.
          const langHint = lang !== 'en' ? lang : undefined
          const { data } = await uploadAPI.uploadAudioAndTranscribe(blob, langHint)

          // Store the audio as data URI (for complaint submission)
          if (data.public_url) {
            setAudioDataUri(data.public_url)
          }
  ...
  // ── Photo upload — now uses direct multipart POST, no R2 ──────────────────
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const files = Array.from(e.target.files || []).slice(0, 5 - photos.length)
    if (!files.length) return
    setUploading(true)
    ...
    for (const file of files) {
      ...
      try {
        // POST file directly to backend → returns base64 data URI
        ...
        // Fallback: create local object URL so user can still see the photo
        // (won't persist after page reload, but works for immediate submission)
        ...
        <AnimatePresence mode="wait">
              ...
              {isNonEnglish ? (
                <div className="space-y-4">
                  ...
                  <div className="bg-blue-900/20 border border-blue-700/30 rounded-xl p-3 flex items-start gap-2.5">
                    <Languages size={14} className="text-blue-400 mt-0.5 shrink-0" />
                    <p className="text-xs text-blue-300 font-body leading-relaxed">
                      <strong>Both versions are saved.</strong> Your {langMeta?.nativeName} text is the original.
                      The English version is shown to MCD officers and used by AI for classification.
                    </p>
                  </div>
                </div>
          ...
          {step === 3 && (
            <motion.div key="s3"
              ...
              {/* Photos — now uses direct upload, no R2 */}
              <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl">
                <label className="text-sm font-semibold text-slate-300 font-body flex items-center gap-2 mb-3">
                  <Camera size={16} className="text-primary-400" /> Upload Photos ({photos.length}/5)
