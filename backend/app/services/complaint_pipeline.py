...
async def run_pipeline(pool, complaint_id: str):
    ...
    try:
        ...
        await notify_citizen(
            ...
            + (f"and assigned to an officer. Expected by {sla_deadline.strftime('%d %b, %I:%M %p')}." if officer else ""),
            language=lang,
            sms_data={"sla_deadline": sla_deadline.strftime("%d %b %I:%M%p") if officer else ""},
        )
