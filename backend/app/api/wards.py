...
Routes (All Intact!):
  GET /api/wards/                    → List all wards
  GET /api/wards/health/all          → All wards with health scores
  GET /api/wards/digests/history     → Returns digest history
  GET /api/wards/digest/{digest_id}  → Get one digest by ID
  GET /api/wards/{ward_id}           → Get ward detail
"""
...
@router.get("/")
async def list_wards(pool=Depends(get_db)):
    """List all wards for selection dropdowns and cards"""
    # Matches setup_database_v6_fixed.py schema perfectly
    rows = await pool.fetch(
...
async def digest_history(
    ...
    if type == "ward" and entity_id:
        conditions.append(f"d.ward_id = ${len(params)+1}::int")
        params.append(int(entity_id))
    elif type == "zone" and entity_id:
        # Check against the joined wards table OR the digest table's zone_name
        conditions.append(f"(w.zone = ${len(params)+1} OR d.zone_name = ${len(params)+1})")
        params.append(entity_id)
    elif type == "city":
        conditions.append("d.digest_type = 'city'")
        
    where = f"WHERE {' AND '.join(conditions)}" if conditions else ""
    
    # Matches setup_database_v6_fixed.py schema: table is `weekly_digests`
    rows = await pool.fetch(
        f"""SELECT d.*, w.ward_name, w.zone
...
@router.get("/{ward_id}")
async def get_ward(ward_id: int, pool=Depends(get_db)):
    """Get ward detail - Kept your dynamic complaint counting!"""
    row = await pool.fetchrow(
        """SELECT w.*,
                  COUNT(c.complaint_id) FILTER (WHERE c.status != 'resolved') AS open_complaints,
                  COUNT(c.complaint_id) FILTER (WHERE c.status = 'resolved') AS resolved_total
